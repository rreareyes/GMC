...
def clean(path) -> None:
    ...
    for i in range(raw_trials["trial"].iat[-1] + 1):
        ...
        evaluated_trial["x_rolling"] = evaluated_trial.x_delta.rolling(10, 10).sum()
        evaluated_trial["y_delta"]   = evaluated_trial.y.diff().shift(0)
        evaluated_trial["y_rolling"] = evaluated_trial.y_delta.rolling(10, 10).sum()
        evaluated_trial["v_angle"]   = np.mod(np.arctan2(evaluated_trial.y_rolling / 10, evaluated_trial.x_rolling / 10), 2 * pi)
        ...
        elif len(evaluated_trial[rejection_bounds(evaluated_trial["v_angle"])]) == 1:
            total_removed += 1
...
for iFile in FILES_MOUSE:
    ...
    print(str(total_removed) + " trials removed from " + iFile)
