...
save(controlled_combinations,
     file = file.path(folder_keys, paste(time_stamp, "controlled_ratios_13.RData", sep = "-")))
...
save(experiment_sequences,
     file = file.path(folder_keys,
                      paste(time_stamp,
                            "experiment-ratio13-sequences.RData", sep = "-")))
write_csv(experiment_sequences,
          file = file.path(folder_keys,
                           paste(time_stamp,
                                 "experiment-ratio13-sequences.csv", sep = "-")))
...
for (iSequence in 1:n_sequences) {
  ...
  difference_plots[[iSequence]] <- ggplot(good_sequences[[iSequence]]) +
    ...
    ggtitle(paste("Sequence", iSequence, sep = " "),
            subtitle = "The minimal distance is 0.13 and 13 for p and rw. \nThere are 100 conditions, irregular number of trials per condition") +

    geom_hline(aes(yintercept = 0)) +

    geom_vline(aes(xintercept = 0)) +

    geom_point(aes(x = p_delta,
                   y = rw_delta,
                   color = as.factor(ratio_cond)),
               alpha = 0.2,
               #width = 0.005,
               #color = "darkblue",
               size  = 2) +

    scale_x_continuous(name = "Difference in probability", breaks = seq(-65, 65, 13)) +

    scale_y_continuous(name = "Difference in reward", breaks = seq(-65, 65, 13)) +
...
#      file = file.path(folder_keys,
#                       paste(time_stamp,
#                             "difference_plots_ratio13.RData", sep = "-")))
